...
export default function Onboarding({ theme, onComplete }) {
  ...
  if (step===0) return wrap(<>
    <div style={{textAlign:"center",marginBottom:28}}>
      ...
      <h1 style={{fontSize:28,fontWeight:600,color:T.textPrimary,marginBottom:14,fontFamily:"'DM Serif Display',serif",lineHeight:1.2}}>Welcome to Northstar</h1>
      <p style={{fontSize:15,color:T.textSecondary,lineHeight:1.7,marginBottom:16}}>Most people overestimate what they can do in a day and underestimate what they can do in 5 years. Northstar helps you close that gap.</p>
      <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:20,textAlign:"left"}}>
        {[
          {icon:"👁", label:"Define your vision", sub:"Who do you want to be in 5 years, across every area of life"},
          {icon:"🎯", label:"Set real goals", sub:"Break your vision into milestones you can actually reach"},
          {icon:"✓",  label:"Build daily discipline", sub:"Habits and tasks that move you forward, tracked every day"},
        ].map(item=>(
          <div key={item.label} style={{display:"flex",gap:12,alignItems:"flex-start",background:T.card,border:`0.5px solid ${T.border}`,borderRadius:10,padding:"12px 14px"}}>
      ...
      {/* Peterson quote — first half */}
      <div style={{background:T.card,border:`0.5px solid ${T.border}`,borderRadius:12,padding:"16px 18px",marginBottom:20,borderLeft:`3px solid ${T.accent}`,borderRadius:"0 12px 12px 0",textAlign:"left"}}>
        <p style={{fontSize:14,color:T.textPrimary,lineHeight:1.7,fontStyle:"italic",marginBottom:8}}>"What sort of life would you have to have to bear your suffering nobly?"</p>
  ...
  if (step===1) {
    ...
    return wrap(<>
      ...
      {/* Show video and second Peterson quote on first area only */}
      {isFirst && (<>
        <div style={{background:T.card,border:`0.5px solid ${T.border}`,borderRadius:12,padding:"14px 16px",marginBottom:14,borderLeft:`3px solid ${T.accent}`,borderRadius:"0 12px 12px 0"}}>
          <p style={{fontSize:14,color:T.textPrimary,lineHeight:1.7,fontStyle:"italic",marginBottom:8}}>"If you could have what you wanted and needed, what might that look like?"</p>
          <span style={{fontSize:12,fontWeight:500,color:T.accent}}>— Jordan Peterson</span>
        </div>
        <a href={PETERSON_VIDEO} target="_blank" rel="noreferrer" style={{display:"flex",alignItems:"center",gap:12,background:T.card,border:`0.5px solid ${T.border}`,borderRadius:12,padding:"12px 14px",marginBottom:16,textDecoration:"none"}}>
          <div style={{width:72,height:48,borderRadius:8,overflow:"hidden",flexShrink:0,background:"#000",position:"relative"}}>
            <img src="https://img.youtube.com/vi/NX2ep5fCJZ0/mqdefault.jpg" alt="13 Minutes To Change Your Life" style={{width:"100%",height:"100%",objectFit:"cover"}} onError={e=>{e.target.style.display="none";}}/>
  ...
  if (step===2) return wrap(<>
    ...
    <p style={{fontSize:13,color:T.textSecondary,marginBottom:6,lineHeight:1.5}}>For each life area, set one concrete target for this year. You can add 6-month and custom timeframe goals anytime in the Goals screen.</p>
    ...
    <div style={{background:T.surface,borderRadius:10,padding:"10px 14px",marginBottom:16,display:"flex",gap:10,alignItems:"flex-start"}}>
      <span style={{fontSize:16,flexShrink:0}}>💡</span>
      <p style={{fontSize:12,color:T.textSecondary,lineHeight:1.5}}>You can add <strong>6-month goals</strong> and <strong>custom timeframe goals</strong> (e.g. for a specific event or race) anytime from the Goals screen.</p>
    </div>
    <button onClick={next} style={{width:"100%",padding:14,background:T.accent,color:T.accentBg,border:"none",borderRadius:10,fontSize:15,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>Next: Habits →</button>
  ...
  if (step===3) {
    ...
    return wrap(<>
      ...
      {allHabits.map((h,i)=>{
        ...
        return (
          <div key={h.name+i} onClick={()=>{if(h.isCustom) return;setSelectedHabits(s=>{const n=new Set(s);isSel?n.delete(h.name):n.add(h.name);return n;});}} style={{display:"flex",alignItems:"center",gap:12,background:T.card,border:`1px solid ${isSel?T.accent:T.border}`,borderRadius:10,padding:"12px 14px",marginBottom:8,cursor:"pointer",transition:"border-color 0.15s",userSelect:"none"}}>
            <div style={{width:22,height:22,borderRadius:"50%",border:`2px solid ${isSel?T.accent:T.borderMed}`,background:isSel?T.accent:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,transition:"all 0.15s"}}>
              {isSel&&<span style={{color:T.accentBg,fontSize:12}}>✓</span>}
            </div>
            <div style={{flex:1}}>
  ...
  if (step===4) return wrap(<>
    ...
    {tasks.map((t,i)=>(
      <div key={i} style={{display:"flex",alignItems:"center",gap:10,background:T.card,border:`0.5px solid ${T.border}`,borderRadius:10,padding:"11px 14px",marginBottom:8}}>
        ...
        <div style={{flex:1}}><div style={{fontSize:13,color:T.textPrimary}}>{t.name}</div><div style={{fontSize:11,color:T.textHint}}>Due: {t.due}</div></div>
