...
const ALL_QUOTES = [
  ...
  { text:"Good. The best thing that can happen to you is when something goes wrong. Because now you get to fix it.", author:"Jocko Willink", source:"", tag:"Resilience" },
  // Frankl
  ...
  { text:"Everything can be taken from a man but one thing: the last of the human freedoms — to choose one's attitude in any given set of circumstances.", author:"Viktor Frankl", source:"Man's Search for Meaning", tag:"Meaning" },
...
const WATCH_LINKS = [
  { title:"Jordan Peterson — Future Authoring", sub:"Map your life 5 years forward", emoji:"🎯", url:"https://www.youtube.com/watch?v=jS2wGBTQmgk" },
  { title:"David Goggins — Unbreakable Mindset", sub:"On discipline and accountability", emoji:"💪", url:"https://www.youtube.com/watch?v=Bc9hOXGXiuk" },
  { title:"Jocko Willink — Extreme Ownership", sub:"Take responsibility for everything", emoji:"⚡", url:"https://www.youtube.com/watch?v=ljqra3BcqWM" },
  { title:"Viktor Frankl — Why to Believe in Others", sub:"On meaning and suffering", emoji:"🕯", url:"https://www.youtube.com/watch?v=fD1512_XJEw" },
];
  ...
  return (
    <div>
      ...
      <div style={{ margin:"14px 16px 0" }}>
        <div onClick={()=>setFeatIdx(i=>(i+1)%FEATURED.length)} style={{ borderRadius:14, padding:"22px 20px 18px", background:`linear-gradient(135deg,${feat.gradient[0]},${feat.gradient[1]})`, minHeight:180, display:"flex", flexDirection:"column", justifyContent:"space-between", cursor:"pointer", position:"relative", overflow:"hidden" }}>
          ...
          {notes.map(n=>(
            <div key={n.id} style={{ background:S.card, border:`0.5px solid ${S.border}`, borderRadius:10, padding:"12px 14px", marginBottom:8, borderLeft:`2px solid ${S.accent}` }}>
              ...
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:8 }}>
                ...
                <button onClick={()=>handleDeleteNote(n.id)} style={{ background:"none", border:"none", cursor:"pointer", color:S.textHint, fontSize:13, padding:0, fontFamily:"inherit" }}>✕</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {WATCH_LINKS.map(w=>(
        <a key={w.title} href={w.url} target="_blank" rel="noreferrer" style={{ display:"flex", alignItems:"center", gap:12, background:S.card, border:`0.5px solid ${S.border}`, borderRadius:12, margin:"0 16px 8px", padding:"13px 14px", textDecoration:"none" }}>
          <div style={{ width:40, height:40, borderRadius:8, background:S.surface, display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, flexShrink:0 }}>{w.emoji}</div>
          <div style={{ flex:1, minWidth:0 }}>
            <div style={{ fontSize:13, fontWeight:500, color:S.textPrimary, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{w.title}</div>
      ...
      {/* Show more button — blends into last card */}
      {!showAllQuotes && filtered.length>INITIAL_QUOTE_COUNT && (
        <div style={{ margin:"0 16px 16px", position:"relative" }}>
          {/* Fade overlay */}
          <div style={{ position:"absolute", top:-60, left:0, right:0, height:60, background:`linear-gradient(transparent, ${S.bg})`, pointerEvents:"none" }} />
          <button onClick={()=>setShowAllQuotes(true)} style={{ width:"100%", padding:"12px", background:S.card, border:`0.5px solid ${S.borderMed}`, borderRadius:12, fontSize:13, fontWeight:500, color:S.accent, cursor:"pointer", fontFamily:"inherit", transition:"all 0.15s" }}>
      ...
      {saved.size>0&&(
        <>
          ...
          {[...saved].filter(i=>i>=0).map(i=>{
            const q=ALL_QUOTES[i];
            if(!q) return null;
            return (
              <div key={i} style={{ background:S.card, border:`0.5px solid ${S.accent}`, borderRadius:12, margin:"0 16px 8px", padding:"14px 16px" }}>
                <span style={{ display:"inline-block", fontSize:10, padding:"2px 8px", borderRadius:20, background:S.surface, color:S.textHint, marginBottom:8 }}>{q.tag}</span>
                <div style={{ fontSize:14, color:S.textPrimary, lineHeight:1.65, fontStyle:"italic" }}>{q.text}</div>
