...
export default function HabitsScreen({ theme, onOpenSettings }) {
  ...
  function handleToggleHabit(id) {
    ...
    if (!wasDone) {
      ...
      if (updated) {
        ...
        if (milestone) setCelebration({ habit:updated.name, streak:milestone });
      }
    }
  }
    ...
    <div>
      ...
      {celebration && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:24 }}>
          <div style={{ background:S.card, borderRadius:16, padding:28, textAlign:"center", maxWidth:320, width:"100%" }}>
            <NorthstarLogo size={52} isDark={S.isDark}/>
            <h2 style={{ fontSize:22, fontWeight:600, color:S.textPrimary, margin:"12px 0 8px" }}>{celebration.streak} day streak!</h2>
            <p style={{ fontSize:14, color:S.textSecondary, lineHeight:1.6, marginBottom:20 }}>
              You've kept up <strong>{celebration.habit}</strong> for {celebration.streak} days in a row. That's not a habit anymore — that's who you are.
            </p>
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.7)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:24 }}>
          <div style={{ background:S.card, borderRadius:16, padding:24, maxWidth:320, width:"100%" }}>
            <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:16 }}>
              <NorthstarLogo size={20} isDark={S.isDark}/>
              <span style={{ fontSize:11, fontWeight:500, textTransform:"uppercase", letterSpacing:"0.1em", color:S.accent }}>Today's progress</span>
            </div>
            <div style={{ background:S.surface, borderRadius:12, padding:16, marginBottom:16 }}>
              ...
              <div style={{ fontSize:13, color:S.textSecondary, marginBottom:12 }}>{dayName}</div>
              ...
              <div style={{ marginTop:12, fontSize:11, color:S.textHint, textAlign:"center", fontStyle:"italic" }}>Northstar</div>
            </div>
            <p style={{ fontSize:12, color:S.textHint, marginBottom:16, textAlign:"center" }}>Screenshot this to share!</p>
      ...
      <div style={{ background:S.card, borderBottom:`0.5px solid ${S.border}`, padding:"14px 20px 12px" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:4 }}>
          ...
          <div style={{ display:"flex", gap:8, alignItems:"center" }}>
            <button onClick={() => setShareVisible(true)} style={{ background:"none", border:"none", cursor:"pointer", padding:0, display:"flex", alignItems:"center" }}>
              <svg width="20" height="20" viewBox="0 0 100 100" fill="none">
                <path d="M50 10 L50 65" stroke={S.textHint} strokeWidth="8" strokeLinecap="round"/>
                <path d="M30 30 L50 10 L70 30" stroke={S.textHint} strokeWidth="8" strokeLinecap="round" strokeLinejoin="round" fill="none"/>
                <path d="M20 55 L20 82 Q20 88 26 88 L74 88 Q80 88 80 82 L80 55" stroke={S.textHint} strokeWidth="8" strokeLinecap="round" fill="none"/>
              </svg>
            </button>
      ...
      {habits.length === 0 && (
        <div style={{ margin:"0 16px 8px", background:S.surface, borderRadius:10, padding:"14px 16px", borderLeft:`3px solid ${S.amber}` }}>
          <div style={{ fontSize:13, color:S.textSecondary }}>No habits set yet. Add your daily disciplines in Settings.</div>
      ...
      {activeTasks.length===0 && !showTaskForm && (
        <div style={{ margin:"0 16px 8px", background:S.surface, borderRadius:10, padding:"12px 14px", borderLeft:`3px solid ${S.blue}` }}>
          <div style={{ fontSize:13, color:S.textSecondary }}>No tasks for today. What's one thing you could do to move closer to your goals?</div>
