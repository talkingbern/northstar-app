...
function PieChart({ pct, color, size=52 }) {
  const r=size/2-4, cx=size/2, cy=size/2, c=2*Math.PI*r, dash=(pct/100)*c;
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      ...
      <text x={cx} y={cy} textAnchor="middle" dominantBaseline="central" fontSize={size<50?10:12} fontWeight="500" fill={color}>{pct}%</text>
    </svg>
  );
}
...
function LineGraph({ pcts, accentColor }) {
  ...
  if (n<2) return <div style={{ height:60,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,color:"#888" }}>Check off some habits to see your trend</div>;
  ...
  return <div dangerouslySetInnerHTML={{__html:`<svg viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:auto"><defs><linearGradient id="lg3" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="${accentColor}" stop-opacity="0.25"/><stop offset="100%" stop-color="${accentColor}" stop-opacity="0.02"/></linearGradient></defs>${grid}<path d="${area}" fill="url(#lg3)"/><path d="${line}" fill="none" stroke="${accentColor}" stroke-width="2" stroke-linejoin="round" stroke-linecap="round"/>${dots}${xl}</svg>`}} />;
}
...
function HistoryScreen({ theme, onBack }) {
  ...
  function getRangeStats(days) {
    if(!days.length||!habits.length) return {pct:0,best:null,worst:null,bestDay:null,worstDay:null};
    const {best,worst}=getBestWorstHabitInRange(days[0],days[days.length-1]);
    const {best:bestDay,worst:worstDay}=getBestWorstDayInRange(days[0],days[days.length-1]);
    const dailyPcts=days.map(d=>getCompletionForDay(d));
    const pct=Math.round(dailyPcts.reduce((a,b)=>a+b,0)/dailyPcts.length);
  ...
  const DailyTab = () => {
    ...
    return (<div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:4,padding:"0 16px"}}>
        ...
        {days30.map(ds=>{
          const pct=getCompletionForDay(ds),isFuture=ds>todayStr,isSel=selected===ds,col=pctColor(pct);
          return <div key={ds} onClick={()=>!isFuture&&setSelected(isSel?null:ds)} style={{aspectRatio:"1",borderRadius:6,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:2,cursor:isFuture?"default":"pointer",background:isSel?S.accent:isFuture?"transparent":S.card,border:`0.5px solid ${isSel?S.accent:S.border}`,opacity:isFuture?0.25:1}}>
            <span style={{fontSize:10,fontWeight:500,color:isSel?S.accentBg:S.textPrimary}}>{new Date(ds+"T12:00").getDate()}</span>
            {!isFuture&&habits.length>0&&<div style={{width:5,height:5,borderRadius:"50%",background:isSel?S.accentBg:col}}/>}
          </div>;
        })}
      </div>
      {selected&&(()=>{
        ...
        return <div style={{margin:"14px 16px 0",background:S.card,border:`0.5px solid ${S.border}`,borderRadius:12,padding:16}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
            <div><div style={{fontSize:15,fontWeight:500,color:S.textPrimary}}>{label}</div><div style={{fontSize:12,color:S.textSecondary,marginTop:2}}>{done} of {total} habits · {pct}%</div></div>
          ...
          {habitDetails.map(h=><div key={h.id} style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
          ...
          {tasksCompleted.length>0&&<><div style={{fontSize:11,fontWeight:500,textTransform:"uppercase",letterSpacing:"0.08em",color:S.textHint,margin:"10px 0 8px"}}>Tasks completed</div>
          ...
          {dayNotes.length>0&&<><div style={{fontSize:11,fontWeight:500,textTransform:"uppercase",letterSpacing:"0.08em",color:S.textHint,margin:"10px 0 8px"}}>Notes</div>
  ...
  const WeeklyTab = () => {
    ...
    return (<div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,padding:"0 16px"}}>
        {weeks.map(w=>{
          const {pct}=getRangeStats(w.days),isSel=selected===w.startStr,col=pctColor(pct);
          return <div key={w.startStr} onClick={()=>setSelected(isSel?null:w.startStr)} style={{background:isSel?S.accent:S.card,border:`0.5px solid ${isSel?S.accent:S.border}`,borderRadius:10,padding:"10px 0",display:"flex",flexDirection:"column",alignItems:"center",cursor:"pointer",gap:6}}>
            <PieChart pct={pct} color={isSel?S.accentBg:col} size={48}/>
      ...
      {selected&&(()=>{
        ...
        return <div style={{margin:"14px 16px 0",background:S.card,border:`0.5px solid ${S.border}`,borderRadius:12,padding:16}}>
          ...
          <div style={{display:"flex",gap:8,marginBottom:14,justifyContent:"space-between"}}>
            {week.days.map(d=>{ const dp=getCompletionForDay(d),dl=new Date(d+"T12:00").toLocaleDateString("default",{weekday:"short"});
              return <div key={d} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3}}><PieChart pct={dp} color={pctColor(dp)} size={36}/><span style={{fontSize:9,color:S.textHint}}>{dl}</span></div>; })}
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            ...
            ].map(s=><div key={s.label} style={{background:S.surface,borderRadius:8,padding:"10px 11px"}}><div style={{fontSize:11,color:S.textHint,marginBottom:3}}>{s.label}</div><div style={{fontSize:12,fontWeight:500,color:S.textPrimary,lineHeight:1.3}}>{s.val}</div></div>)}
          </div>
        </div>;
      })()}
    </div>);
  ...
  const MonthlyTab = () => {
    ...
    return (<div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,padding:"0 16px"}}>
        {months.map(m=>{
          const {pct}=getRangeStats(m.days),isSel=selected===m.startStr,col=pctColor(pct);
          return <div key={m.startStr} onClick={()=>setSelected(isSel?null:m.startStr)} style={{background:isSel?S.accent:S.card,border:`0.5px solid ${isSel?S.accent:S.border}`,borderRadius:10,padding:"12px 0",display:"flex",flexDirection:"column",alignItems:"center",cursor:"pointer",gap:8}}>
            <PieChart pct={pct} color={isSel?S.accentBg:col} size={52}/>
      ...
      {selected&&(()=>{
        ...
        return <div style={{margin:"14px 16px 0",background:S.card,border:`0.5px solid ${S.border}`,borderRadius:12,padding:16}}>
          ...
          <div style={{display:"flex",gap:10,marginBottom:14}}>
            {weeks.map((wDays,i)=>{ const wp=wDays.map(d=>getCompletionForDay(d)),wa=Math.round(wp.reduce((a,b)=>a+b,0)/wp.length);
              return <div key={i} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4}}><PieChart pct={wa} color={pctColor(wa)} size={52}/><span style={{fontSize:10,color:S.textHint}}>Wk {i+1}</span></div>; })}
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            {[{label:"🏆 Best habit",val:best?`${best.habit.name} (${best.pct}%)`:"-"},
              {label:"📉 Worst habit",val:worst?`${worst.habit.name} (${worst.pct}%)`:"-"},
...
export default function ProgressScreen({ theme }) {
  ...
  // Reload data every time the screen is visible or tab changes
  ...
  useEffect(() => {
    reload();
    // Also reload on window focus (user switches back from Today screen)
  ...
  const rangeDays  = getDaysInRange(rangeStart, todayStr);
  ...
  const dailyPcts = rangeDays.map(d=>{ const done=habits.filter(h=>habitLog[d]&&habitLog[d][h.id]).length; return habits.length>0?Math.round((done/habits.length)*100):0; });

  // Area breakdown — live
  const areaBreakdown = (() => {
    const areas={};
    habits.forEach(h=>{ if(!areas[h.area]) areas[h.area]={done:0,total:0}; rangeDays.forEach(d=>{ areas[h.area].total++; if(habitLog[d]&&habitLog[d][h.id]) areas[h.area].done++; }); });
      ...
      {hasData&&(<>
        ...
        <div style={{background:S.card,border:`0.5px solid ${S.border}`,borderRadius:12,margin:"10px 16px 0",padding:14}}>
          ...
          {tooltipVisible&&<div style={{background:S.surface,border:`0.5px solid ${S.borderMed}`,borderRadius:8,padding:"9px 11px",fontSize:12,color:S.textSecondary,lineHeight:1.5,marginBottom:12}}>
            Progress is calculated from your daily habit completion rate for each life area {periodLabel}. The more consistently you show up, the higher the progress.
          </div>}
          {areaBreakdown.length===0
            ...
            : areaBreakdown.map(a=>(
              <div key={a.area} style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{height:4,background:S.surface,borderRadius:2,marginTop:5}}><div style={{height:4,borderRadius:2,background:aFill(a.area),width:`${a.pct}%`,transition:"width 0.4s"}}/></div>
                </div>
